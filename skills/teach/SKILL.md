...
2. **Establish.** 
   ...
   - If the node is a system, or adds a part to one — see *Systems get a Mermaid diagram* — draw its diagram here, before the details, and explain against it.
	- If the node has a planned visual learning moment, search for the real image now, inspect the previews, and import only one that shows the planned feature clearly. Broaden the search phrase if needed, while keeping the planned visible feature and learning goal fixed. Do not substitute a related but different object (for example, flat LGA contacts for BGA solder balls). Put a passing image beside the explanation, point to the exact observation, and connect that observation to the concept. If no preview passes, teach the node without a web image.
